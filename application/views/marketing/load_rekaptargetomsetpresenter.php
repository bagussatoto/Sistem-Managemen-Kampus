<table class="table  table-striped table-sm table-styling">
  ...
  <tbody>
    <?php
      ...
      foreach($rekap as $r){
        ...
        $totalrealisasi = $totalrealisasi + $r->omset;
        $totpresent = $totalrealisasi / $totaltarget * 100;
        $persentase = $r->omset / $r->jumlah * 100;
        $ta         = $r->ta_mkt;
        $bulannow   = date('m');
        $tahunnow   = date('Y');
        if($r->order<=3)
     ...
     <td align="right"><?php echo number_format($target,'0','','.')." ".$ket; ?></td>
  ...
  <thead>
    <tr class="table-inverse">
      <th>TOTAL</th>
      <th class="text-right"><?php if(!empty($totaltarget)){echo number_format($totaltarget,'0','','.');  }?></th>
      <th class="text-right"><?php if(!empty($totalrealisasi)){echo number_format($totalrealisasi,'0','','.');  }?></th>
